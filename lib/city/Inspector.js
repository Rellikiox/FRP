...
Inspector = (function() {
  ...
  Inspector.prototype._set_state = function(new_state) {
    console.log("Transitioning from " + this.label + " to " + new_state);
    this.label = new_state;
    return this.current_state = this['s_' + new_state];
  };

  Inspector.prototype._move = function(point) {
    this._face_point(point);
    return this.forward(this.speed);
  };

  Inspector.prototype._face_point = function(point) {
    var heading, turn;
  ...
  Inspector.prototype._angle_between_points = function(point_a, point_b) {
    var dx, dy;
  ...
  Inspector.prototype._in_point = function(point) {
    return 0.1 > ABM.util.distance(this.x, this.y, point.x, point.y);
  };

  Inspector.prototype._get_path_to = function(point) {
...
NodeInspector = (function(_super) {
  ...
  NodeInspector.prototype._get_close_nodes = function() {
    ...
    nodes = [];
    if (this.p.node != null) {
      nodes_to_check = RoadNode.road_nodes.inRadius(this.p.node, 10);
    } else {
      nodes_to_check = RoadNode.road_nodes.inRadius(this, 10);
    }
...
RoadInspector = (function(_super) {
  ...
  RoadInspector.prototype.s_go_to_inspection_point = function() {
    ...
    if (this.path == null) {
      this.path = this._get_path_to(this.inspection_point);
    }
    this._move(this.path[0]);
    if (this._in_point(this.path[0])) {
      this.path.shift();
