...
var HouseInspector, Inspector, NodeInspector, PlotInspector, RoadInspector,
...
Inspector = (function() {
  ...
  Inspector.spawn_road_inspector = function(patch) {
    var inspector;
    inspector = this.spawn_inspector(patch, RoadInspector);
    inspector.init();
    return inspector;
  };
  ...
  Inspector.spawn_house_inspector = function(patch) {
    var inspector;
    inspector = this.spawn_inspector(patch, HouseInspector);
...
NodeInspector = (function(_super) {
  ...
  NodeInspector.prototype._inspect_node = function(node) {
    var crosses_plot, patch, path, _i, _len;
...
RoadInspector = (function(_super) {
  ...
  RoadInspector.prototype.s_get_inspection_point = function() {
    ...
    if (this._valid_point(this.inspection_point)) {
  ...
  RoadInspector.prototype.s_increment_radius = function() {
    if (House.houses_below_minimum() && this.nodes_built_board.message_count() === 0) {
      this.ring_radius += this.ring_increment;
      return this._set_state('get_inspection_point');
    }
  };

  RoadInspector.prototype._valid_point = function(point) {
  ...
  RoadInspector.prototype._is_valid_construction_point = function(patch) {
    ...
    return road_dist > Road.too_connected_threshold && ((construction_dist == null) || construction_dist > 2);
  };
...
PlotInspector = (function() {
  ...
  PlotInspector.prototype._get_plot = function(patch) {
    var closed_list, edge, n, open_list, p, _i, _len, _ref;
    closed_list = [];
    open_list = [patch];
    edge = false;
...
HouseInspector = (function() {
  function HouseInspector() {}

  HouseInspector.ticks_per_report = 100;

  HouseInspector.prototype.init = function(house) {
    this.house = house;
    this.hidden = true;
    this._set_initial_state('report_state');
    return this.msg_board = MessageBoard.get_board('population_needs');
  };

  HouseInspector.prototype.s_report_state = function() {
    this.msg_board.post_message({
      'type': 'hospital',
      'value': this.house.hospital_distance,
      'population': this.house.citizens
    });
    this.ticks_since_report = 0;
    return this._set_state('wait');
  };

  HouseInspector.prototype.s_wait = function() {
    if (this.ticks_since_report > HouseInspector.ticks_per_report) {
      return this._set_state('report_state');
    } else {
      return this.ticks_since_report += 1;
    }
  };

  return HouseInspector;
