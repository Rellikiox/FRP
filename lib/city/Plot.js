...
House = (function() {
  House.hsl_color = [1.0, 1.0, 0.5];

  House.max_citizens = 10;

  House.minimum_housing_available = 0.5;

  House.expansion_threshold = 0.5;

  House.expansion_factor = 1.25;
...
GenericBuilding = (function() {
  GenericBuilding.hospital_color = [0.44, 1.0, 0.5];

  GenericBuilding.school_color = [0.50, 1.0, 0.5];

  GenericBuilding.store_color = [0.56, 1.0, 0.5];

  GenericBuilding.info = {
    hospital: {
      threshold: 400,
      radius: 20,
      hsl_color: GenericBuilding.hospital_color,
      rgb_color: Colors.hslToRgb.apply(Colors, GenericBuilding.hospital_color).map(function(f) {
        return Math.round(f);
      }),
      size: '2x2'
    },
    school: {
      threshold: 100,
      radius: 10,
      hsl_color: GenericBuilding.school_color,
      rgb_color: Colors.hslToRgb.apply(Colors, GenericBuilding.school_color).map(function(f) {
        return Math.round(f);
      }),
      size: '2x1'
    },
    store: {
      threshold: 50,
      radius: 5,
      hsl_color: GenericBuilding.store_color,
      rgb_color: Colors.hslToRgb.apply(Colors, GenericBuilding.store_color).map(function(f) {
        return Math.round(f);
      }),
      size: '1x1'
    }
  ...
  GenericBuilding._get_shape = {
    ...
    '2x2': function(patch) {
      var available, offset, offsets, shape, _check_patch, _get_patch;
      _get_patch = function(patch, offset) {
        return CityModel.get_patch_at({
          x: patch.x + offset.x,
          y: patch.y + offset.y
        });
      };
      _check_patch = function(patch, offset) {
      ...
      available = (function() {
        ...
        for (_i = 0, _len = offsets.length; _i < _len; _i++) {
          offset = offsets[_i];
          if (!offset.some(function(offset) {
            return !_check_patch(patch, offset);
          })) {
      ...
      if (available.length > 0) {
        ...
        return (function() {
          ...
          for (_i = 0, _len = shape.length; _i < _len; _i++) {
            offset = shape[_i];
            _results.push(_get_patch(patch, offset));
          }
          return _results;
        })();
      }
  ...
  GenericBuilding.get_shape = function(patch, type) {
    var size;
    size = this.info[type].size;
    return this._get_shape[size](patch);
  };

  GenericBuilding.make_here = function(blocks, subtype) {
    var block, _i, _len, _results;
    if (!blocks.some(function(b) {
      return !Block.is_block(b);
    })) {
      _results = [];
      for (_i = 0, _len = blocks.length; _i < _len; _i++) {
        ...
        _results.push(block.building = new GenericBuilding(blocks, subtype));
      }
      return _results;
    }
  };
  ...
  GenericBuilding.fits_here = function(patch, type) {
    var _ref;
    return ((_ref = this.get_shape(patch, type)) != null ? _ref.length : void 0) > 0;
  };
  ...
  function GenericBuilding(blocks, building_subtype) {
    var block, _i, _len;
    this.blocks = blocks;
    ...
    for (_i = 0, _len = blocks.length; _i < _len; _i++) {
      block = blocks[_i];
      block.color = GenericBuilding.info[this.building_subtype].rgb_color;
    }
  }
