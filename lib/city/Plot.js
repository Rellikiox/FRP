...
House = (function() {
  ...
  House.prototype.distances = null;
  ...
  House.prototype.reallocate_citizens = function() {
    ...
    for (i = _i = 0, _ref = this.citizens; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
      House.population -= 1;
      _results.push(HouseBuilder.spawn_house_builder(this, Plot.get_available_block()));
    }
    return _results;
  };

  House.prototype.dist_to_need = function(need) {
  ...
  House.prototype.set_dist_to_need = function(need, dist) {
...
GenericBuilding = (function() {
  ...
  GenericBuilding.prototype.init = function(building_subtype) {
    this.building_subtype = building_subtype;
    return this._set_distances();
  };

  GenericBuilding.prototype._set_distances = function() {
    var blocks_in_radius, house, house_dist, m_dist, _i, _len, _results;
    blocks_in_radius = Block.blocks.inRadius(this, 10);
    _results = [];
    for (_i = 0, _len = blocks_in_radius.length; _i < _len; _i++) {
      house = blocks_in_radius[_i];
      if (!(House.has_house(house))) {
        continue;
      }
      m_dist = this._manhatan_distance_to(house);
      house_dist = house.dist_to_need(this.building_subtype);
      if ((house_dist == null) || house_dist > m_dist) {
        _results.push(house.set_dist_to_need(this.building_subtype, m_dist));
      } else {
        _results.push(void 0);
      }
    }
