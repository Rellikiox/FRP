...
RoadNode = (function() {
  ...
  RoadNode.split_link_at = function(road) {
    var downstream_node, node, upstream_node;
    ...
    return CityModel.link_agents(node, downstream_node);
  };
  ...
  RoadNode._find_upstream_node = function(road) {
    while (road.node == null) {
      road = Road._get_max_neighbour(road, "dist_to_city_hall", {
        filter: function(p) {
          var _ref;
          return Road.is_road(p) && !((_ref = p.node) != null ? _ref.creating : void 0);
        }
      });
    }
    return road.node;
  ...
  RoadNode._find_downstream_node = function(road) {
    var n_road, _i, _len, _ref;
    _ref = road.n4;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      n_road = _ref[_i];
      if (Road.is_road(n_road)) {
        if ((n_road.node != null) && !n_road.node.creating) {
          road = n_road;
        }
      }
    }
...
RoadNode_instance_properties = {
  ...
  _any_neighbours_nodes: function() {
